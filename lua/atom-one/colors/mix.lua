-- Palette for `atom-one-mix`.
-- Ground truth: `OneDark-Pro-mix.json` in
-- github.com/Binaryify/OneDark-Pro (MIT).
--
-- UI chrome matches the base theme almost exactly; the one distinguishing
-- value is `focusBorder`, which the source sets as a semi-transparent
-- `#3e4452c3` (blended here onto editor.background rather than truncated to
-- its RGB part, to stay faithful to the actual rendered color). Mix's
-- `tokenColors` genuinely differ in count from the base theme (277 vs 275
...
function M.setup()
  ...
  local c = {
    ...
    border_highlight = Util.blend("#3e4452", 0xc3 / 0xff, bg), -- focusBorder (8-digit in source)
